...
pub fn generate_js_bindings(
    ...
    let helpers = [
        ("__fox_panic", "        __fox_panic: (v) => { throw new Error('Fox Panic: ' + v); },\n"),
        ("__fox_str_starts_with", "        __fox_str_starts_with: (s, prefix) => (s != null && prefix != null && s.startsWith(prefix)) ? 1 : 0,\n"),
        ("__fox_str_ends_with", "        __fox_str_ends_with: (s, suffix) => (s != null && suffix != null && s.endsWith(suffix)) ? 1 : 0,\n"),
        ("__fox_str_contains", "        __fox_str_contains: (s, sub) => (s != null && sub != null && s.includes(sub)) ? 1 : 0,\n"),
        ("__fox_str_index_of", "        __fox_str_index_of: (s, sub) => (s != null && sub != null) ? s.indexOf(sub) : -1,\n"),
        ("__fox_str_last_index_of", "        __fox_str_last_index_of: (s, sub) => (s != null && sub != null) ? s.lastIndexOf(sub) : -1,\n"),
        ("__fox_str_is_empty", "        __fox_str_is_empty: (s) => (s == null || s.length === 0) ? 1 : 0,\n"),
        ("__fox_str_eq", "        __fox_str_eq: (a, b) => (a === b) ? 1 : 0,\n"),
        ("__fox_str_join", "        __fox_str_join: (a, b) => (a == null ? '' : a) + (b == null ? '' : b),\n"),
        ("__fox_str_compare", "        __fox_str_compare: (a, b) => {\n            if (a == null && b == null) return 0;\n            if (a == null) return -1;\n            if (b == null) return 1;\n            if (a < b) return -1;\n            if (a > b) return 1;\n            return 0;\n        },\n"),
        ("__fox_dom_is_null", "        __fox_dom_is_null: (r) => r == null || r === undefined,\n"),
        ("__fox_dom_is_null_str", "        __fox_dom_is_null_str: (s) => s == null || s === undefined,\n"),
        ("__fox_dom_element_append_child", "        __fox_dom_element_append_child: (parent, child) => parent.appendChild(child),\n"),
        ("__fox_dom_element_set_attribute", "        __fox_dom_element_set_attribute: (el, name, value) => el.setAttribute(name, value),\n"),
        ("__fox_dom_element_get_attribute", "        __fox_dom_element_get_attribute: (el, name) => el.getAttribute(name),\n"),
        ...
        ("__fox_dom_console", "        __fox_dom_console: (level, msg) => {\n            switch (level) {\n                case 1: console.log(msg); break;\n                case 2: console.info(msg); break;\n                case 3: console.warn(msg); break;\n                case 4: console.error(msg); break;\n                case 5: console.debug(msg); break;\n                default: console.log(msg);\n            }\n        },\n"),
        ("__fox_dom_performance_now", "        __fox_dom_performance_now: () => performance.now(),\n"),
        ("__fox_time_now", "        __fox_time_now: () => BigInt(Date.now()),\n"),
        ("__fox_time_local_offset", "        __fox_time_local_offset: () => new Date().getTimezoneOffset(),\n"),
        ("__fox_f64_to_str", "        __fox_f64_to_str: (v) => String(v),\n"),
        ("__fox_i32_to_str", "        __fox_i32_to_str: (v) => String(v),\n"),
        ("__fox_i64_to_str", "        __fox_i64_to_str: (v) => String(v),\n"),
        ("__fox_json_parse_int", "        __fox_json_parse_int: (s) => parseInt(s, 10),\n"),
        ("__fox_json_parse_float", "        __fox_json_parse_float: (s) => parseFloat(s),\n"),
        ("__fox_json_parse_string", "        __fox_json_parse_string: (s) => JSON.parse(s),\n"),
        ("__fox_json_encode_string", "        __fox_json_encode_string: (s) => JSON.stringify(s),\n"),
        ("__fox_str_substring", "        __fox_str_substring: (s, start, end) => s.substring(start, end),\n"),
        ("__fox_http_send", "        __fox_http_send: WebAssembly.Suspending ? new WebAssembly.Suspending(async (url, method, headersRaw, body) => {\n            try {\n                const headers = {};\n                if (headersRaw) {\n                    const lines = headersRaw.split('\\n');\n                    for (const line of lines) {\n                        if (!line) continue;\n                        const idx = line.indexOf(':');\n                        if (idx !== -1) {\n                            const key = line.slice(0, idx).trim();\n                            const val = line.slice(idx + 1).trim();\n                            headers[key] = val;\n                        }\n                    }\n                }\n                const options = {\n                    method: method || 'GET',\n                    headers: headers\n                };\n                if (method !== 'GET' && method !== 'HEAD' && body !== undefined && body !== null) {\n                    options.body = body;\n                }\n                const res = await fetch(url, options);\n                const resBody = await res.text();\n                return {\n                    status: res.status,\n                    statusText: res.statusText,\n                    body: resBody\n                };\n            } catch (err) {\n                return {\n                    status: 0,\n                    statusText: err.message || String(err),\n                    body: ''\n                };\n            }\n        }) : () => ({ status: 0, statusText: 'No Suspending support', body: '' }),\n"),
        ("__fox_http_get_status", "        __fox_http_get_status: (r) => r ? r.status : 0,\n"),
        ("__fox_http_get_status_text", "        __fox_http_get_status_text: (r) => r ? r.statusText : '',\n"),
        ("__fox_http_get_body", "        __fox_http_get_body: (r) => r ? r.body : '',\n"),
        ("__fox_async_yield", "        __fox_async_yield: WebAssembly.Suspending ? new WebAssembly.Suspending(() => new Promise(resolve => queueMicrotask(resolve))) : () => {},\n"),
        ("__fox_async_sleep", "        __fox_async_sleep: WebAssembly.Suspending ? new WebAssembly.Suspending((ms) => new Promise(resolve => setTimeout(resolve, ms))) : (ms) => {},\n"),
        ("__fox_queue_task", "        __fox_queue_task: (task) => {\n            if (rawExports.fox_run_task) {\n                if (WebAssembly.promising) {\n                    queueMicrotask(() => WebAssembly.promising(rawExports.fox_run_task)(task));\n                } else {\n                    rawExports.fox_run_task(task);\n                }\n            }\n        },\n"),
    ];
    ...
    for (id, lit) in string_literals.iter().enumerate() {
        std_env.push_str(&format!("        s{}: {},\n", id, serde_json::to_string(lit).unwrap_or_else(|_| "\"\"".to_string())));
    }
